...
export default function SupplierForm({ initialData, supplierId }: Props) {
  ...
  const [form, setForm] = useState<SupplierFormData>({ ...emptyForm, ...initialData })
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className={labelClass}>仕入先コード</label>
            <input type="text" name="code" value={form.code} onChange={handleChange} className={fieldClass} placeholder="例: S-0001" />
          </div>
          <div>
      ...
      <div className="flex gap-3">
        <button type="submit" disabled={saving}
          ...
          {saving ? '保存中...' : supplierId ? '更新する' : '登録する'}
        </button>
