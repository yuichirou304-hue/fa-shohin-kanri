...
interface OrderItem {
  ...
  quantity: number; unit: string; unit_price: number; amount: number; comment1: string; comment2: string
}
interface Order {
  id: string; order_number: string; order_date: string; customer_order_number: string; notes: string
  items: OrderItem[]
...
export default function PrintOrderPage() {
  ...
  useEffect(() => {
    // orderId から project_id を取得するため、別APIを使う
    // ここでは受注IDから直接取得するAPIを呼ぶ
    fetch(`/api/print/order/${params.orderId}`)
      .then(r => r.json())
      .then(setOrder)
  }, [params.orderId])

  if (!order) return <div style={{ padding: 40, fontFamily: 'sans-serif' }}>読み込み中...</div>

  const totalAmount = order.items.reduce((s, i) => s + (Number(i.amount) || 0), 0)
  const customerName = order.project?.customer?.company_name || order.project?.customer_name || ''
  ...
  return (
    <>
      ...
      <div style={{ padding: '20px 30px', maxWidth: 900, margin: '0 auto' }}>
        <h1 style={{ textAlign: 'center', fontSize: 20, marginBottom: 24, letterSpacing: 8 }}>受　注　書</h1>
              ...
              <td style={{ border: 'none', width: '50%', verticalAlign: 'top', paddingLeft: 20 }}>
                <table style={{ borderCollapse: 'collapse', fontSize: 11 }}>
                  <tbody>
                    ...
                    <tr>
                      <td style={{ border: 'none', paddingRight: 8 }}>受注日</td>
                      <td style={{ border: 'none' }}>{order.order_date || '-'}</td>
                    </tr>
                    <tr>
                      <td style={{ border: 'none', paddingRight: 8 }}>見積番号</td>
                      <td style={{ border: 'none' }}>{order.project?.quote_number || '-'}</td>
                    </tr>
                    <tr>
                      <td style={{ border: 'none', paddingRight: 8 }}>客先発注No</td>
                      <td style={{ border: 'none' }}>{order.customer_order_number || '-'}</td>
                    </tr>
                  </tbody>
                </table>
              </td>
